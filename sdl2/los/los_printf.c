...
int los_sprintf_low(char *buf, const char *fmt, uint8_t va_num)
{
    char *str;
    ...
    for (str = buf; *fmt; fmt++)
    {
        ...
        {
            fmt++;
            field_width = *(uint32_t *)los_get_arg(va_num++);
            if (field_width < 0)
            {
        ...
        if ('.' == *fmt)
        {
            ...
            {
                ++fmt;
                precision = *(uint32_t *)los_get_arg(va_num++);
            }
            if (precision < 0)
                precision = 0;
        }
        ...
        case 'p':
        {
            ...
            str = number(str, *(uint32_t *)los_get_arg(va_num++), 16, field_width, precision, flags);
            continue;
        }
        case 'n':
        {
            if ('l' == qualifier)
            {
                long *ip = *(uint32_t *)los_get_arg(va_num++);
                *ip = (str - buf);
            }
            else
            {
                int *ip = *(uint32_t *)los_get_arg(va_num++);
                *ip = (str - buf);
            }
            continue;
        }
        ...
        case 'a':
        {
            if ('l' == qualifier)
            {
                str = eaddr(str, *(uint32_t *)los_get_arg( va_num++), field_width, precision, flags);
            }
            else
            {
                str = iaddr(str, *(uint32_t *)los_get_arg( va_num++), field_width, precision, flags);
            }
            continue;
        }
        ...
        if (qualifier == 'l')
        {
            num = *(uint32_t *)los_get_arg(va_num++);
        }
        else if (qualifier == 'h')
        {
            if (flags & SIGN)
                num =*( int16_t *)los_get_arg( va_num++);
            else
                num = *(uint16_t *)los_get_arg(va_num++);
        }
        else if (flags & SIGN)
        {
            num = *(uint32_t *)los_get_arg( va_num++);
